...
import { Plus, Search, Trash2, FileText, Image as ImageIcon, ChevronRight, Upload, Loader2, Save, X, Check, AlertCircle } from 'lucide-react';
        ...
        <div className="flex h-[calc(100vh-2rem)] gap-6 text-gray-100 overflow-hidden bg-gray-950 p-4 font-sans relative">
                    ...
                    <div className="bg-gray-900 border border-white/10 p-6 rounded-2xl shadow-2xl w-full max-w-sm transform scale-100 transition-all">
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-400">Bu işlem geri alınamaz ve blog yazısı kalıcı olarak silinir.</p>
                            </div>
            ...
            <aside className="w-1/3 min-w-[300px] flex flex-col bg-gray-900/60 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden shadow-xl">
                <div className="p-4 border-b border-white/10 flex items-center justify-between bg-black/40">
                    ...
                    <button onClick={() => setSelectedId('new')} className="p-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg transition-all shadow-[0_0_15px_rgba(8,145,178,0.4)]">
                        <Plus size={18} />
                    </button>
                </div>
                <div className="p-4 pb-2 relative">
                    <Search className="absolute left-7 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                    <input type="text" placeholder="Ara..." className="w-full bg-black/50 border border-white/10 rounded-lg py-2 pl-9 pr-4 text-sm focus:outline-none focus:border-cyan-500/50 text-gray-300" />
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                    {blogs.length === 0 && <div className="text-center text-gray-500 mt-10 text-sm">Liste boş.</div>}
                    {blogs.map((blog) => (
                        <div key={blog.id} onClick={() => setSelectedId(blog.id)} className={`group p-3 rounded-xl border cursor-pointer transition-all ${String(selectedId) === String(blog.id) ? 'bg-cyan-900/20 border-cyan-500/50 shadow-[0_0_10px_rgba(6,182,212,0.1)]' : 'border-transparent hover:bg-white/5'}`}>
                            <div className="flex justify-between items-start mb-1">
                                ...
                                {String(selectedId) === String(blog.id) && <ChevronRight size={16} className="text-cyan-500 animate-pulse" />}
                            </div>
            ...
            <main className="flex-1 bg-gray-900/60 backdrop-blur-md border border-white/10 rounded-2xl flex flex-col overflow-hidden relative shadow-xl">
                <header className="p-4 border-b border-white/10 flex items-center justify-between bg-black/40">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={handleSave} className="p-2 px-4 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg flex items-center gap-2 text-sm font-semibold shadow-[0_0_15px_rgba(8,145,178,0.4)]">
                            <Save size={18} /> Kaydet
                        </button>
                    </div>
                </header>
                    ...
                    <div className="max-w-4xl mx-auto space-y-6 pb-10">
                        ...
                        >
                            ...
                            {uploading ? (
                                <div className="flex flex-col items-center text-cyan-400 z-10">
                                    <Loader2 size={48} className="animate-spin mb-3 drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]" />
                                    <span className="text-sm font-semibold tracking-wider animate-pulse">GÖRSEL YÜKLENİYOR...</span>
                                </div>
                            ) : formData.image ? (
                                <>
                                    <img src={formData.image} alt="Kapak" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                                ...
                                <div className="flex flex-col items-center text-gray-500 group-hover:text-cyan-400 transition-colors duration-300">
                                    ...
                                    <span className="text-base font-bold text-gray-400 group-hover:text-white transition-colors">Kapak Görseli Yükle</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-2 space-y-2">
                                ...
                                <input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Başlık..." className="w-full bg-black/50 border border-gray-700 focus:border-cyan-500 text-lg font-bold text-white p-3 rounded-xl outline-none transition-all focus:bg-black/70" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">Kategori</label>
                                <input type="text" name="category" value={formData.category} onChange={handleInputChange} placeholder="Kategori..." className="w-full bg-black/50 border border-gray-700 focus:border-cyan-500 text-white p-3 rounded-xl outline-none transition-all focus:bg-black/70" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-2 h-[500px] flex flex-col">
                            ...
                            <div className="flex-1 rounded-xl overflow-hidden shadow-inner ring-1 ring-white/5 border border-white/5">
                                <Editor key={selectedId} value={formData.content || ''} onChange={handleContentChange} />
                            </div>
                        </div>

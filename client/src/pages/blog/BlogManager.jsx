...
import {
    Plus, Search, Trash2, FileText, Image as ImageIcon, ChevronRight, Upload, Loader2, Save, Check, AlertCircle
} from 'lucide-react';
import Editor from '../../components/Editor';
import api from '../../services/api';
import {collection, addDoc, doc, setDoc, deleteDoc, getDocs} from "firebase/firestore";
import {db} from '../../firebase';
import DatabaseSwitch from '../../components/DatabaseSwitch.jsx';
...
export default function BlogManager() {
    ...
    const fetchBlogs = async () => {
        ...
        try {
            ...
            const apiBlogs = (Array.isArray(apiRes.data) ? apiRes.data : []).map(b => ({...b, dbSource: 'postgres'}));

            const firestoreBlogs = [];
            if (firestoreSnapshot && firestoreSnapshot.docs) {
                firestoreSnapshot.forEach(doc => {
                    firestoreBlogs.push({id: doc.id, ...doc.data(), dbSource: 'firestore'});
                });
            }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const errorMessage = e.message || e.response?.data?.message || "Kaydetme hatası oluştu.";
    ...
    const filteredBlogs = blogs.filter(blog => blog.title && blog.title.toLowerCase().includes(searchTerm.toLowerCase()));
    return (<div
        className="flex h-[calc(100vh-2rem)] gap-6 text-gray-100 overflow-hidden bg-gray-950 p-4 font-sans relative">

        {toast && (<div className={`absolute top-6 right-6 z-50 px-4 py-3 rounded-lg shadow-2xl flex items-center gap-3 animate-fade-in-down border backdrop-blur-md transition-all duration-300
        ...
        <aside
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                ...
                {filteredBlogs.map((blog) => (<div key={blog.id} onClick={() => handleSelectBlog(blog.id)}
                ...
                <div className="max-w-4xl mx-auto space-y-6 pb-10">
                    ...
                    <div
                        ...
                        className={`
                                ...
                                ${formData.image ? 'border-cyan-500/30 bg-black/40' : 'border-gray-700 hover:border-cyan-500/50 bg-black/20 hover:bg-black/30'}
                    ...
                    >
                        ...
                        {uploading ? (<div className="flex flex-col items-center text-cyan-400 z-10">
                            <Loader2 size={48}
                                     className="animate-spin mb-3 drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]"/>
                            <span
                                className="text-sm font-semibold tracking-wider animate-pulse">GÖRSEL YÜKLENİYOR...</span>
                        </div>) : formData.image ? (<>
                    ...
                    <div className="space-y-2 h-[500px] flex flex-col">
                        ...
                        <div className="flex-1 rounded-xl overflow-hidden shadow-inner ...">
                            {isLoading ? (<div className="flex items-center justify-center h-full text-cyan-500">

...
// İkonlar
import { User, Mail, Lock, Camera, Save, Loader2, ShieldCheck, Calendar as CalendarIcon, ChevronDown, AlertCircle, Check } from 'lucide-react';
...
export default function Profile() {
    ...
    useEffect(() => {
        const fetchBlogCount = async () => {
            ...
            try {
                ...
                const userBlogs = response.data.filter(blog => blog.author?.id === user.id);
    ...
    const handleAvatarChange = async (e) => {
        ...
        try {
            ...
            // 2. Eski resmi sil (Eğer varsa ve default değilse)
            const oldImage = user.image || user.avatar;
            if (oldImage) {
                const oldFilename = oldImage.split('/').pop();
                if (oldFilename && !oldImage.includes('dicebear') && !oldImage.includes('google')) {
                    try {
            ...
            // Backend { user: { ... } } formatında bekliyor
    ...
    // --- STİLLER ---
    const labelClass = "text-xs font-bold text-gray-400 ml-1 uppercase tracking-wider mb-1 block";
    ...
    return (
        <div className="max-w-5xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* Toast Bildirimi */}
            {toast && (
                <div className={`fixed top-24 right-8 z-50 px-6 py-4 rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.3)] backdrop-blur-md flex items-center gap-3 animate-in slide-in-from-right border transition-all duration-300
                    ${toast.type === 'error'
                        ? 'bg-red-500/10 border-red-500 text-red-400'
                        : 'bg-green-500/10 border-green-500 text-green-400'}`}>
                    {toast.type === 'error' ? <AlertCircle size={24} /> : <ShieldCheck size={24} />}
                    <div>
                        <h4 className="font-bold">{toast.type === 'error' ? 'Hata!' : 'Başarılı!'}</h4>
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Hesap bilgilerinizi ve tercihlerinizi yönetin.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <div className="bg-gray-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6 text-center relative overflow-hidden">
                        ...
                        <h2 className="text-xl font-bold text-white mb-1">{user?.username || 'Kullanıcı'}</h2>
                        <p className="text-cyan-400 text-sm mb-6">{user?.email}</p>

                        <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-6">
                            <div className="text-center">
                                <span className="block text-2xl font-bold text-white">{blogCount}</span>
                                <span className="text-xs text-gray-500 uppercase tracking-wider">Blog</span>
                            </div>
                            <div className="text-center border-l border-white/10">
                                <span className="block text-2xl font-bold text-white">2.4k</span>
                                <span className="text-xs text-gray-500 uppercase tracking-wider">Okunma</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <form onSubmit={handleSubmit} className="bg-gray-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-8 space-y-8 shadow-xl">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    ...
                                    <div className={inputWrapperClass}>
                                        ...
                                        <input type="text" name="username" value={formData.username} onChange={handleChange} placeholder="Kullanıcı Adı" className={inputClass} />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className={inputWrapperClass}>
                                        <User size={18} className={iconClass} />
                                        <input type="text" name="firstName" value={formData.firstName} onChange={handleChange} placeholder="Adınız" className={inputClass} />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className={inputWrapperClass}>
                                        <User size={18} className={iconClass} />
                                        <input type="text" name="lastName" value={formData.lastName} onChange={handleChange} placeholder="Soyadınız" className={inputClass} />
                                    </div>
                                </div>
                                <div className="md:col-span-2">
                                    ...
                                    <div className={inputWrapperClass}>
                                        ...
                                        <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="ornek@mail.com" className={inputClass} />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <div className={inputWrapperClass}>
                                        ...
                                        >
                                            ...
                                            <option value="other" className="bg-gray-900">Belirtmek İstemiyorum</option>
                                        </select>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <div className={inputWrapperClass}>
                                        ...
                                        <input type="password" name="newPassword" value={formData.newPassword} onChange={handleChange} placeholder="Değiştirmek istemiyorsanız boş bırakın" className={inputClass} />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className={inputWrapperClass}>
                                        <Lock size={18} className={iconClass} />
                                        <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="Yeni şifreyi tekrar girin" className={inputClass} />
                                    </div>
                                </div>
                            </div>
                        </div>

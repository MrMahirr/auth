...
const Toolbar = () => {
    ...
    return (
        <div className="flex flex-wrap gap-1 p-2 border-b border-white/10 bg-black/40 sticky top-0 z-10">
            ...
            <ToolbarBtn icon={UnderlineIcon} onClick={() => formatText('underline')} isActive={isUnderline}
                        title="Altı Çizili"/>
            <ToolbarBtn icon={Strikethrough} onClick={() => formatText('strikethrough')} isActive={isStrikethrough}
                        title="Üstü Çizili"/>
            <div className="w-px h-6 bg-white/10 mx-1 self-center"></div>
            ...
            <ToolbarBtn icon={AlignLeft} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'left')}
                        title="Sola Hizala"/>
            <ToolbarBtn icon={AlignCenter} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'center')}
                        title="Ortala"/>
            <ToolbarBtn icon={AlignRight} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, 'right')}
                        title="Sağa Hizala"/>
            <div className="w-px h-6 bg-white/10 mx-1 self-center"></div>

            <ToolbarBtn icon={List} onClick={() => editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined)}
                        title="Liste"/>
            <ToolbarBtn icon={ListOrdered}
                        onClick={() => editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined)}
                        title="Sıralı Liste"/>
            <ToolbarBtn icon={Quote} onClick={formatQuote} title="Alıntı"/>
            <ToolbarBtn icon={Eraser} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, '')}
                        title="Temizle"/>
        </div>
    );
};
...
const HtmlPlugin = ({initialHtml}) => {
    ...
    useEffect(() => {
        ...
        editor.update(() => {
            ...
            if (isLoadedRef.current && currentContent.trim().length > 0) return;
...
export default function Editor({value, onChange}) {
    console.log("Editor rendered with value:", value ? value.substring(0, 20) + "..." : "empty");
    const initialConfig = {
        namespace: 'MyEditor',
        theme,
        onError(error) {
                    ...
                    <OnChangePlugin
                        onChange={(editorState, editor) => {
                            editorState.read(() => {
                                ...
                                if (html !== value) {
                                    console.log("OnChangePlugin triggering change. html:", html.substring(0, 20) + "...", "value:", value ? value.substring(0, 20) + "..." : "empty");
                                    onChange(html);
                                }
                            });
                        }}

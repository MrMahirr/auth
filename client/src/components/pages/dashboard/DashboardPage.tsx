...
const StatCard = ({ title, value, icon: Icon, color, trend }: StatCardProps) => (
    <div className="relative overflow-hidden rounded-xl border border-gray-800 bg-gray-900/60 p-6 backdrop-blur-md transition-all hover:border-gray-600 hover:shadow-[0_0_20px_rgba(0,0,0,0.3)] group">
        <div
            className={`absolute -right-6 -top-6 h-24 w-24 rounded-full opacity-10 blur-2xl transition-all group-hover:opacity-20 ${color}`}
        ></div>
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div className="flex gap-3">
                    <Button
                        label="Rapor İndir"
                        icon="pi pi-download"
                        className="p-button-outlined !border-gray-700 !text-gray-300 hover:!bg-gray-800 hover:!text-white !text-sm"
                    />
                    <Button
                        label="Sistemi Yenile"
                        icon="pi pi-refresh"
                        className="!bg-cyan-600 !border-none hover:!bg-cyan-500 !text-sm shadow-[0_0_15px_rgba(8,145,178,0.5)]"
                    />
                </div>
            </div>
                ...
                <div className="rounded-xl border border-gray-800 bg-gray-900/60 p-6 backdrop-blur-md flex flex-col gap-6">
                    ...
                    <div className="space-y-4">
                        {[1, 2, 3].map((item) => (
                            <div
                                key={item}
                                className="group flex items-center justify-between rounded-lg bg-black/40 p-4 border border-gray-800 hover:border-cyan-500/50 transition-colors cursor-pointer"
                            >
                                <div className="flex items-center gap-3">
                                    <div
                                        className={`h-2 w-2 rounded-full ${
                                            item === 3 ? "bg-red-500 animate-pulse" : "bg-green-500"
                                        }`}
                                    ></div>
                                    <div>
                    ...
                    <div className="mt-auto rounded-lg bg-gradient-to-r from-purple-900/50 to-blue-900/50 p-4 border border-white/10">
                        ...
                        <p className="text-xs text-gray-400 mb-3">
                            GPU kaynaklarınız %80 kapasiteye ulaştı.
                        </p>
                        <Button
                            label="Yükselt"
                            className="w-full !bg-white/10 !border-white/20 hover:!bg-white/20 !text-xs"
                        />
                    </div>
                </div>
            </div>
